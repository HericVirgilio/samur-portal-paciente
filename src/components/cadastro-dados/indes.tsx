...
export default function Dados({ sendDataToParent }: Props) {
  ...
  useEffect(() => {
    ...
    const carregarNoticias = async () => {
      try {
        const response = await axios.get<Nacionalidade[]>(
          "http://192.168.1.90:3030/nacionalidade"
        );
  ...
  const EnviarFormulario = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (nacionalidade === "Brasileiro") {
      if (validadeCpf === true) {
        ...
        setErroDados(
          "Existem campos incorretos, é necessário fazer as correções antes de enviar !"
        );
      }
    } else {
      if (cpf !== "") {
    ...
    <div className="BoxFormDados">
      <form className="DivDadosForm" onSubmit={EnviarFormulario}>
        <FormControl className="FormControl" defaultChecked>
          <InputLabel id="escolaridade-label" size="small">
            Nacionalidade
          </InputLabel>
          <Select
            size="small"
          ...
          >
            {nacionalidadeData?.map((objeto) => (
              <MenuItem key={objeto.nacionalidade} value={objeto.nacionalidade}>
